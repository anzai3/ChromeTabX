...
document.addEventListener('click',async event=>{
 ...
 try {
  ...
  document.getElementById('preview-body').textContent=data.state==='done'?data.excerpt:copy().unavailable;
...
hover.append(hoverTitle,hoverNote,hoverBody);document.body.append(hover);
let hoverCard=null,hoverTimer,hideTimer,hoverEpoch=0;
...
async function showHover(card){
 ...
 const epoch=++hoverEpoch;hoverCard=card;
 ...
 try {
  const tab=await getTab(Number(node.dataset.summary),node);const data=await read(tab);
  if(epoch!==hoverEpoch || !card.isConnected)return;
  if(live && (await chrome.tabs.get(tab.id)).url!==tab.url)throw Error('changed');
  if(epoch!==hoverEpoch)return;
  hoverBody.textContent=data.state==='done'?data.excerpt:copy().unavailable;
...
document.addEventListener('click',hideHover,true);
