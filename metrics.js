...
export async function updateMetrics() {
 ...
 try {
  ...
  if(!data?.ok || !Number.isFinite(data.memoryBytes) || !Number.isFinite(data.cpuPercent))throw Error('采集失败');
  $('chrome-memory').textContent=(data.memoryBytes/1024**3).toFixed(2)+' GB';
  $('chrome-cpu').textContent=data.cpuPercent.toFixed(1)+'%';
  ...
  $('chrome-memory').textContent='—';$('chrome-cpu').textContent='—';
