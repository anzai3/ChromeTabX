...
function render() {
 ...
 $('#content').innerHTML = groups.size ? [...groups].map(([name,items],index)=>`<section class="group" style="${colorStyle(name)}"><div class="group-heading"><span class="dot"></span><h3>${esc(name)}</h3><span class="number">${String(items.length).padStart(2,'0')}</span><span class="line"></span></div><div class="cards">${items.map(t=>`<article class="card" style="${colorStyle(titleGroups.get(t.id)||'其他',view.startsWith('sub:')?view.slice(4):'')}"><div class="card-top"><span class="favicon">${esc(domain(t.url).replace(/^www\./,'')[0]?.toUpperCase() || '↗')}</span><button class="tab-open" data-open="${t.id}" title="${esc(t.title)} · ${esc(t.url)}"><span class="tab-title">${esc(titleSubject(t.title) || '未命名标签')}</span></button><button class="close-tab card-close" data-close="${t.id}" aria-label="关闭 ${esc(t.title)}" title="关闭标签"><svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" focusable="false"><path d="M6 6l12 12M18 6L6 18" fill="none" stroke="currentColor" stroke-width="2"/></svg></button></div>${query.trim() && bodyMatches.get(t.id)?.url===t.url && bodyMatches.get(t.id)?.matched ? `<p class="match-snippet"><span>正文命中</span> ${esc(bodyMatches.get(t.id).snippet)}</p>` : ''}<p class="page-summary" data-language-ui data-summary="${t.id}" data-summary-state="loading" data-url="${esc(t.url)}" data-title="${esc(t.title)}"></p><div class="card-bottom"><img class="tab-thumbnail" data-thumbnail="${t.id}" data-url="${esc(t.url)}" alt="" width="160" height="90" loading="lazy" decoding="async" hidden>${dupSet.has(t.id)?'<span class="badge">重复</span>':''}<div class="tab-actions"><button class="tab-action preview-button" data-language-ui data-preview="${t.id}" aria-haspopup="dialog"><span aria-hidden="true">◉</span><span class="action-label">预览</span></button></div></div></article>`).join('')}</div></section>`).join('') : `<div class="empty"><strong>${query.trim()?(searchBusy?'正在搜索正文…':'没有找到匹配的标签。'):view==='duplicates'?'没有重复标签。':'这里暂时没有标签。'}</strong><p>${query?'换个关键词试试。':view==='duplicates'?'没有发现相同网址的重复页面。':view==='inactive'?'没有符合此时长的标签，可以缩短筛选时长。':'打开一些网页，或者选择其他视图。'}</p></div>`;
