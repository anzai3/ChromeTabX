...
ChromeTabX 在本地处理打开标签的标题、网址、状态和可读取正文，用于搜索、分类与标题提取。不再截取或缓存网页缩略图。语言偏好保存在本地，卸载扩展后删除。
