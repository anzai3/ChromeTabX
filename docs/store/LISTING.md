...
No account, no analytics, no developer upload of your browsing data. Search and thumbnail processing happen on your device. Previews and background page loading still contact the original websites.

Thumbnails are generated gradually and may be unavailable for some pages. Chrome may display a debugging notice during background capture. Some sites block embedded previews or require you to open the original tab. Browser-internal pages cannot be read. The optional macOS memory monitor requires a separately installed helper; all tab-management features work without it.
...
无需注册，没有分析追踪，也不会向开发者上传浏览数据。搜索和缩略图处理在本机完成；预览和后台加载页面仍会访问原网站。

缩略图逐步生成，部分网页可能无法生成。后台截图时 Chrome 可能显示调试提示。部分网站禁止嵌入预览或需要打开原标签；浏览器内部页面无法读取。可选的 macOS 内存监控需要另行安装本地辅助程序，不影响标签管理功能。
...
| alarms | Resume the bounded background thumbnail queue after service-worker suspension. |
| debugger | Temporarily attach to an eligible tab and call Page.captureScreenshot to capture its viewport without selecting it; detach afterward. This is used only for page thumbnails. |
...
5. Allow background capture to run. Eligible pages receive cached thumbnails; unsupported/failed pages show status. This behavior still needs fresh browser verification before submission.
