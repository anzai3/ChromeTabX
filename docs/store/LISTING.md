...
• Open Gmail, DeepSeek and Feishu beside search; customize each shortcut or drag a web link onto its icon.
...
No account, no analytics, no developer upload of your browsing data. Search processing happen on your device. Previews still contact the original websites.

Some sites block embedded previews or require you to open the original tab. Browser-internal pages cannot be read. The optional macOS memory monitor requires a separately installed helper; all tab-management features work without it.
...
• 搜索标题、网址和已加载页面的可读取正文。
• 集中查看所有 Chrome 窗口的标签。
• 搜索框旁默认显示 Gmail、DeepSeek 和飞书，每个都能自定义，也可拖入网址生成图标入口。
• 按自动生成的主题分类浏览。
• 先预览网页，再切换到原标签。
• 检查重复网址，关闭多余页面。
• 支持英文和简体中文，默认跟随系统语言。
...
无需注册，没有分析追踪，也不会向开发者上传浏览数据。搜索处理在本机完成；预览仍会访问原网站。

部分网站禁止嵌入预览或需要打开原标签；浏览器内部页面无法读取。可选的 macOS 内存监控需要另行安装本地辅助程序，不影响标签管理功能。
...
5. Confirm cards have no screenshot area or thumbnail-update banner; Preview remains available.
