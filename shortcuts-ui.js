...
const nameInput=dialog.querySelector('#shortcut-name'),urlInput=dialog.querySelector('#shortcut-url'),iconInput=dialog.querySelector('#shortcut-favicon'),notice=dialog.querySelector('[role="status"]');
...
function render(){
 ...
 resolveSlots(defaults,custom).forEach((site,index)=>{
  ...
  if(site){
   ...
   loadShortcutIcon(icon,fallback,iconCandidates(site.url,iconTabs,globalThis.chrome?.runtime,site.favicon));
 ...
 dialog.querySelector('#shortcut-icon-hint').textContent=strings.t('iconHint');
 dialog.querySelector('h2').textContent=strings.t('heading');
 for(const key of ['name','url','favicon'])dialog.querySelector(`[data-label="${key}"]`).textContent=strings.t(key);
 for(const key of ['save','cancel','reset'])dialog.querySelector(`[data-action="${key}"]`).textContent=strings.t(key);
}
function edit(index){slot=index;const site=resolveSlots(defaults,custom)[index];nameInput.value=site?.name||'';urlInput.value=site?.url||'';iconInput.value=site?.favicon||'';notice.textContent='';dialog.showModal();nameInput.focus();}
async function save(reset=false){
 ...
 const favicon=iconInput.value.trim()?safeUrl(iconInput.value.trim()):'';
 if(!reset&&favicon===null){notice.textContent=strings.t('iconInvalid');return;}
 const key=`shortcut-slot-${slot}`;
 try {
  if(!storage)throw Error('No extension storage');
  if(reset)await storage.remove(key);else await storage.set({[key]:{name,url,favicon}});
  if(reset)delete custom[slot];else custom[slot]={name,url,favicon};render();dialog.close();
