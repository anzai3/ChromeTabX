...
卡片缩略图后台逐步生成：单任务逐步处理，每次完成后间隔 2 秒，优先屏幕内卡片和已加载标签。休眠标签会临时加载，截图后恢复休眠（用户正在使用或播放音频时除外），不切换标签。图片压缩为 320×180 WebP，懒加载，缓存最多 512 张、约 8 MB，6 小时有效，关闭浏览器后清除。失败后至少 5 分钟再试。需新增 debugger 和 alarms 权限，Chrome 可能显示调试提示。限速降低开销，但无法保证完全不影响性能。

缩略图队列会保留后台服务重启前的定时任务，打开或刷新管理页不会再推迟生成时间。

缩略图卡片直接读取临时缓存，并显示生成中或失败状态，避免静默留白。若 Chrome 后台仍为旧版本，管理页会显示“应用更新”按钮。

## Chrome 扩展商店上架准备

1.6.5 统一扩展名称为 ChromeTabX，新增[隐私政策](PRIVACY.md)、[中英文商店介绍](docs/store/LISTING.md)和仅包含运行文件的打包脚本。运行 `python3 scripts/package-store.py`，安装包输出到不纳入 Git 的 `releases/` 目录。目前尚未上传或提交审核，详见[提交状态](docs/store/STATUS.md)。
