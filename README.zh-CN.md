...
卡片缩略图后台逐步生成：每 30 秒处理一个，优先已加载标签。休眠标签会临时加载，截图后恢复休眠（用户正在使用或播放音频时除外），不切换标签。图片压缩为 320×180 WebP，懒加载，缓存最多 100 张、30 分钟有效，关闭浏览器后清除。失败后至少 5 分钟再试。需新增 debugger 和 alarms 权限，Chrome 可能显示调试提示。限速降低开销，但无法保证完全不影响性能。
