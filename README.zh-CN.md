...
1.6.6 移除调试接口后台截图，以及 debugger / alarms 权限。仅在你正常浏览当前 Chrome 窗口中的已加载网页时缓存缩略图，不为截图自动加载或切换标签。缓存为 320×180 WebP，最多 512 张、约 8 MB，6 小时有效；没有缓存的页面不显示缩略图。


缩略图卡片直接读取临时缓存，没有缓存时不再显示“生成中”。若 Chrome 后台仍为旧版本，管理页会显示“应用更新”按钮。
