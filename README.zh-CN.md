...
ChromeTabX 将 Chrome 新标签页替换为轻量的标签工作台，帮助你搜索网页、浏览自动分类、关闭重复页面。全部在本机运行，无需账号或云服务。

扩展内也使用 **Tab Matrix · 标签矩阵** 名称。基于原生 JavaScript 和 Manifest V3，加载扩展无需安装依赖或构建。
...
语言选择提供**跟随系统**、**简体中文**和 **English**。不支持的语言回退英文；选择保存在当前设备。网页标题、正文片段及分类关键词保持原文。
