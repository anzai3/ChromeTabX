...
常用网址间距收紧：20px 图标、32px 点击区域、4px 间隙，保持右对齐。

## 1.7.11

拖动快捷图标到其他位置即可调整顺序，自动保存到本机并同步到已打开的管理页。从外部拖入网址仍用于添加或替换。

## 1.7.12

快捷入口增加统一书签栏容器，真实图标和占位符使用相同尺寸的居中格子，统一垂直对齐。

## 1.7.13

侧边栏品牌文字统一为上方 ChromeTabX、下方 Tab Manager，中英文界面保持一致。

## 1.7.14

网页预览加载提示移至顶部，不遮挡内容；悬停等待缩短至 150ms，立即发起网页加载。列表刷新、鼠标移出不再自动关闭，同一预览重复悬停不重新加载。可用关闭按钮、Esc、点击外部或滚动关闭。实际加载速度仍取决于网站及其嵌入限制。

## 1.8.0

设置新增使用时长卡片，显示今日和累计 ChromeTabX 前台使用时间，从本次更新开始统计。本地存储、跨管理页去重，超过 15 秒的中断不计入以避免休眠误计；约每五秒保存，异常退出可能丢失最后一个间隔。此功能不统计各网站浏览时长。
